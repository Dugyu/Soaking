...
// An abstract class that encapsulates geometry data that provides vertex
// attributes and know how to draw itself.
class Geometry {
  public:
    ...
    // by this geometry should bind to. It can be -1 to indicate that this
    // stream is not used. The caller is responsible for enable/disable vertex
    // attribute arrays.
...
class VertexFormat {
  public:
    ...
    struct AttribDesc {
        ...
        AttribDesc(const std::string &_name, GLint _size, GLenum _type,
                   GLboolean _normalized, int _offset)
            : name(_name), size(_size), type(_type), normalized(_normalized),
              offset(_offset) {
    ...
    VertexFormat &put(const std::string &name, GLint size, GLenum type,
                      GLboolean normalized, int offset) {
    ...
    int getVertexSize() const { return vertexSize_; }

    int getNumAttribs() const { return attribDescs_.size(); }

    const AttribDesc &getAttrib(int i) const { return attribDescs_[i]; }

    // returns the index of the atribute with given name, or -1 if the name is
    // not found
    ...
    // Calls glVertexAttribPointer with appropirate arguments to bind the
    // attribute indexed by 'attribIndex' within this VertexFormat to vertex
    // attribute location specified by 'glAttribLocation'
    void setGlVertexAttribPointer(int attribIndex, int glAttribLocation) const {
        ...
        glVertexAttribPointer(glAttribLocation, ad.size, ad.type, ad.normalized,
                              vertexSize_,
...
// Light wrapper for a GL buffer object storing vertices, together with format
// for its vertices.
class FormattedVbo : public GlBufferObject {
  ...
  public:
    ...
    const VertexFormat &getVertexFormat() const { return format_; }
    ...
    void upload(const Vertex *vertices, int length, bool dynamicUsage = false) {
        ...
        if (dynamicUsage) {
            // We always call glBufferData with a NULL ptr here, so that OpenGL
            // knows that we're done with old data, and that if the old vbo is
            // in use, it can  allocate us a new one without stalling the
            // pipeline
...
// Light wrapper for a GL buffer object storing indices, together with format
// for its indices, one of GL_UNSIGNED_BYTE, GL_UNSIGNED_SHORT, or
// GL_UNSIGNED_INT
class FormattedIbo : public GlBufferObject {
  ...
  public:
    // format must be one of GL_UNSIGNED_BYTE, GL_UNSIGNED_SHORT,or
    // GL_UNSIGNED_INT GL_UNSIGNED_SHORT is the default
    FormattedIbo(GLenum format = GL_UNSIGNED_SHORT)
        : format_(format), length_(0) {
        assert(format == GL_UNSIGNED_BYTE || format == GL_UNSIGNED_SHORT ||
               format == GL_UNSIGNED_INT);
    }

    GLenum getIndexFormat() const { return format_; }
...
// A flexible light weight Geometry implementation allowing drawing using
// multiple vertex buffers, with or without an index buffer, and as different
// primitives (e.g., triangles, quads, points...).
//
// This essentially maintains a map of
//   vertex attribute names --> (FormattedVbo, attribute name)
//
// To draw its self, it binds all the vertex attributes that it is wired to, and
// calls the suitable OpenGL calls to draw either indexed or non-index geometry.
// There are optimizations to call glBindBuffer only once for each distince
// FormattedVbo it wires to.

class BufferObjectGeometry : public Geometry {
  public:
    // Initialize, with zero vertex buffer and index buffer attached. Defaults
    // to TRIANGLES primitive
    ...
    // Declares and maps all vertex attributes contained in the 'source'
    // FormattedVbo. Same names are used for each attribute.
    BufferObjectGeometry &wire(shared_ptr<FormattedVbo> source);

    // Set the index buffer to be used. Pass in a null shared_ptr to mean
    // non-indexed. Default is non-indexed
    ...
    bool isIndexed() const { return (bool)ib_; }
  ...
  private:
    typedef std::map<std::string,
                     std::pair<shared_ptr<FormattedVbo>, std::string>>
        Wiring;
    ...
    struct PerVbWiring {
        ...
        // A map from (attribute index in vb) --> relative index within
        // BufferObjectGeometry's exposed vertex attributes
    ...
    // wiringChanged_ is true and we need to draw or return list of vertex
    // attributes.
    void processWiring();
};
...
// First, some predefined vertex formats. It's perfectly easy for you to roll
// your own. Note that we define assignment operator (=) from GenericVertex of
// geometrymaker.h so that we can use geometrymaker on any of these format
...
struct VertexPN {
    ...
    VertexPN(const Cvec3f &pos, const Cvec3f &normal) : p(pos), n(normal) {}
...
struct VertexPNX : public VertexPN {
    ...
    VertexPNX(float x, float y, float z, float nx, float ny, float nz, float u,
              float v)
    ...
    // Define copy constructor and assignment operator from GenericVertex so we
    // can use make* functions from geometrymaker.h
    VertexPNX(const GenericVertex &v) { *this = v; }
...
struct VertexPNTBX : public VertexPNX {
    ...
    VertexPNTBX(float x, float y, float z, float nx, float ny, float nz,
                float tx, float ty, float tz, float bx, float by, float bz,
                float u, float v)
        : VertexPNX(x, y, z, nx, ny, nz, u, v), t(tx, ty, tz), b(bx, by, bz) {}

    VertexPNTBX(const Cvec3f &pos, const Cvec3f &normal, const Cvec3f &tangent,
                const Cvec3f &binormal, const Cvec2f &texCoords)
        : VertexPNX(pos, normal, texCoords), t(tangent), b(binormal) {}

    VertexPNTBX(const Cvec3 &pos, const Cvec3 &normal, const Cvec3 &tangent,
                const Cvec3 &binormal, const Cvec2 &texCoords)
        : VertexPNX(pos, normal, texCoords),
          t(tangent[0], tangent[1], tangent[2]),
          b(binormal[0], binormal[1], binormal[2]) {}

    // Define copy constructor and assignment operator from GenericVertex so we
    // can use make* functions from geometrymaker.h
    VertexPNTBX(const GenericVertex &v) { *this = v; }
...
// A vertex with floating point Position, Normal, Tangent, Binormal, teXture  Coord
struct VertexPNCRX : public VertexPNX {
    Cvec3f c, r; // billboard center, relative location to billboard center frame

    static const VertexFormat FORMAT;

    VertexPNCRX() {}

    VertexPNCRX(float x, float y, float z, float nx, float ny, float nz,
                float cx, float cy, float cz, float rx, float ry, float rz,
                float u, float v)
        : VertexPNX(x, y, z, nx, ny, nz, u, v), c(cx, cy, cz), r(rx, ry, rz) {}

    VertexPNCRX(const Cvec3f &pos, const Cvec3f &normal, const Cvec3f &boardCenter,
                const Cvec3f &localPos, const Cvec2f &texCoords)
        : VertexPNX(pos, normal, texCoords), c(boardCenter), r(localPos) {}

    VertexPNCRX(const Cvec3 &pos, const Cvec3 &normal, const Cvec3 &boardCenter,
                const Cvec3 &localPos, const Cvec2 &texCoords)
        : VertexPNX(pos, normal, texCoords),
          c(boardCenter[0], boardCenter[1], boardCenter[2]),
          r(localPos[0], localPos[1], localPos[2]) {}

    // Define copy constructor and assignment operator from GenericVertex so we
    // can use make* functions from geometrymaker.h
    VertexPNCRX(const GenericVertex &v) { *this = v; }

    VertexPNCRX &operator=(const GenericVertex &v) {
        p = v.pos;
        n = v.normal;
        c = v.tangent;
        r = v.binormal;
...
class SimpleIndexedGeometry : public BufferObjectGeometry {
  ...
  public:
    SimpleIndexedGeometry()
        : vbo(new FormattedVbo(Vertex::FORMAT)),
          ibo(new FormattedIbo(size2IboFmt(sizeof(Index)))) {
        wire(vbo);
        indexedBy(ibo);
        primitiveType(GL_TRIANGLES);
    }

    SimpleIndexedGeometry(const Vertex *vertices, const Index *indices,
    ...
    void upload(const Vertex *vertices, const Index *indices, int numVertices,
                int numIndices) {
        vbo->upload(vertices, numVertices, true);
...
typedef SimpleUnindexedGeometry<VertexPNCRX> SimpleGeometryPNCRX;

typedef SimpleIndexedGeometry<VertexPN, unsigned short> SimpleIndexedGeometryPN;
typedef SimpleIndexedGeometry<VertexPNX, unsigned short>
    SimpleIndexedGeometryPNX;
typedef SimpleIndexedGeometry<VertexPNTBX, unsigned short>
    SimpleIndexedGeometryPNTBX;
typedef SimpleIndexedGeometry<VertexPNCRX, unsigned int>
SimpleIndexedGeometryPNCRX;
